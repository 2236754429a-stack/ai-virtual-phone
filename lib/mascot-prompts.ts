...
export const ADVENTURE_PROMPT = `===== 冒险 App 操作规范 =====

「冒险」是手机里的地图 RPG App。它不是普通角色卡、世界书或自定义 App，不能用其它套件的创建动作代替。

工作流：
1. 用户要查看冒险内容时，先用「列出冒险世界」确认现有世界、生成状态和存档。
2. 用户要新建冒险世界时，收集世界描述；有明确要求时再填写 tone、regionCount、mainQuestType、npcCount、difficulty 和 companionNames。
3. 调用「创建冒险世界」后，系统会自动调用冒险专用世界生成提示词，解析为 tagged-block 格式，生成地图并创建初始存档；不要自己拼 JSON，也不要把冒险内容写进角色卡/世界书。
4. 创建成功后用「导航」打开 mapmode（页面显示名为「冒险」），再告诉用户世界已经保存。
5. companionNames 必须传「读取角色」返回的完整角色名；找不到角色时先停止并让用户确认，不要猜名字。

参数建议：
· regionCount：区域数量，通常 4-8。
· npcCount：NPC 数量，通常 8-20。
· difficulty：例如「轻松」「适中」「困难」。
· mainQuestType：例如「调查」「护送」「生存」「自由发挥」。

冒险世界生成只接受系统内部的标准标签格式；如果生成失败，原始输出会随错误返回，应该如实告诉用户并建议重试，不要声称已经创建成功。`;
...
· checklist 核对：可编辑的输出格式检查清单；装进配方才注入，官方出厂件「官方 · 标准核对」可直接使用
· filter 滤网：正则清洗正文
...
7. 机括要让模型每轮额外输出一段专用内容（记忆行、拍摄单、选项等）时，按四步做：把契约用 sections 挂到 ticket 段，让模型用 [状态栏:名字]...[/状态栏] 输出；在 onRawReply 把这块从原始回复剪走并写入 store；在 onBeforeSend 用 lastReply 把上一轮内容放回最后一条 assistant 历史；再把核对项用 sections 挂到 checklist 段。只在提示词里要求而不回填历史，长局会越来越容易漏写。
