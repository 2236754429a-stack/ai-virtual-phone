...
export const PAGE_GREETINGS: Record<string, string> = {
  ...
  vn: "剧情不错嘛~ 需要帮你想台词吗？",
  mapmode: "进入冒险世界啦！想看看地图情报还是整理队伍日志？",
  desktop: "随时待命~ 角色卡、预设、世界书、正则、CSS，你说了算",
};

export const ADVENTURE_PROMPT = `===== 跑团冒险操作规范 =====

「冒险」是手机里的跑团 RPG 探索与冒险地图系统：玩家可以在不同世界中探索节点、推进主线、触发剧情与奇遇，并与随行 AI 伙伴共同经历故事。小卷对这个系统有完全的管理权限，既可以查阅，也可以创建和改写。

===== 查阅与记录 =====
· 「列出冒险世界」：查看目前已创建的世界清单（世界名、世界观背景、主线任务、更新时间等）。
· 「读取冒险状态」：获取当前世界的最新游玩进度（玩家状态/血量/属性、当前所在节点与区域、主线进度、随行伙伴状态、近期日志与剧情摘要）。
· 「读取冒险日志」：查看该世界过往的冒险日志流水，回顾旅途中的关键事件与选择。
· 「读取地图节点」：按区域查看各节点信息（节点名、描述、危险度、是否已探索/访问过）。
· 「记录冒险手记」：以小卷桌面助手的身份，在玩家当前的冒险存档中添加一条随行手记或随笔点评。

===== 创建新世界 =====
· 「创建冒险世界」：替用户从零设计并创建一个完整世界。把你设计好的世界观**完整写进 description**（时代背景、地理与区域构成、势力与种族、主线冲突构想、基调风格），再按需传 tone / regionCount / npcCount / difficulty / mainQuestType；想陪用户一起玩就传 companionCharacterIds。
  - 引擎会按你的设计自动生成完整世界（区域节点地图、NPC、主线支线、奇遇池）并创建初始存档，生成约需一两分钟，期间请告知用户稍等。
  - 创建完成后主动向用户播报世界名、世界观一句话简介和主线标题。
  - 用户描述很模糊时也可以大胆按你的理解设计，不必反复追问。

===== 改写已有世界 =====
· 「写入世界设定」：把设计好的设定写进已有世界——worldName / lore / mainQuestTitle / mainQuestSynopsis / mainQuestStages（整组替换主线阶段）/ newSideQuests（追加支线）/ regionLore（按区域名更新背景）/ nodeDescriptions（按节点名更新描述）。
  - 只能改文本设定，不能增删区域或节点（那会影响已渲染的地图结构）。
  - 改写前建议先「读取地图节点」「读取冒险状态」了解现状，避免和已有剧情冲突。
· 「删除冒险世界」：删除世界及其全部存档。**危险操作**：必须先向用户确认，用户明确同意后才传 confirm=true 执行。`;
