 ...
 * - Mossland: same-origin proxy → binary mp3/wav audio blob
 */
export async function synthesizeSpeech(
    ...
    if (provider === "Mossland") {
        return synthesizeMossland(text, voiceConfig);
    }
...
async function synthesizeMossland(text: string, config: VoiceApiConfig): Promise<Blob | null> {
    ...
    if (!response.ok) {
        ...
        throw new Error(payload.message || `Mossland TTS 请求失败 (${response.status})`);
    }

    const blob = await response.blob();
    return new Blob([await blob.arrayBuffer()], {
        type: config.mosslandFormat === "wav" ? "audio/wav" : "audio/mpeg",
    });
}
